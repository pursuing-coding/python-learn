...
要输出 \`17 + 5 = 22\` 这样“文字和计算结果混排”的内容，最方便的写法是 **f-string**：在字符串前加 \`f\`，大括号里的表达式会被求值后嵌进文本：
\`\`\`python
print(f"{a} + {b} = {a + b}")   # 17 + 5 = 22
\`\`\`
第 11 章会系统学习它，现在只需要会“往大括号里放变量或算式”即可。
...
累加、累除会频繁写 \`x = x + 1\` 这类语句，Python 提供了简写：\`total += x\` 等价于 \`total = total + x\`，\`n //= 10\` 等价于 \`n = n // 10\`。\`-=\`、\`*=\`、\`%=\` 同理。
...
\`\`\`
这样能精确控制分隔符，也不会残留行尾空格。这里提前用到了**列表**：\`[]\` 建一个空列表，\`append\` 往里加元素，\`join\` 把它们拼成一个字符串——第 9 章会系统学习列表，本章会用这三招就够了。
...
\`\`\`
还记得第 10 章的 \`counts.get(k, 0)\` 吗？把它扩展成 \`counts[k] = counts.get(k, 0) + 1\` 就是手动计数，而 Counter 正是这种写法的工业级封装，并列时按**首次出现顺序**排。
